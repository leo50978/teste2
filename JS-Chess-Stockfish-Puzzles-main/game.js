...
function playSound(type){
    if(type == "start"){
        safeAudioPlay(startSound)
    }else if(type == "move"){
        safeAudioPlay(moveSound)
    }else if(type == "capture"){
        safeAudioPlay(captureSound)
    }else if(type == "check"){
        safeAudioPlay(checkSound)
    }else if(type == "over"){
        safeAudioPlay(overSound)
    }else if(type == "stalemate"){
        safeAudioPlay(stalemateSound)
    }else if(type == "checkmate"){
        safeAudioPlay(checkmateSound)
    }else if(type == "castle"){
        safeAudioPlay(castleSound)
    }
    return
}
...
var depth = 1
var movetime = 1000
var kobposhChessBotProfile = "fo"
var kobposhChessBotPvCandidates = []
var kobposhChessBotMoveCounter = 0
...
function normalizeKobposhChessBotProfile(level){
    var normalized = String(level || "").trim().toLowerCase()
    if(normalized == "weak" || normalized == "easy" || normalized == "amateur" || normalized == "low") return "weak"
    return "fo"
}

function configureKobposhChessBot(options){
    options = options || {}
    kobposhChessBotProfile = normalizeKobposhChessBotProfile(options.level || options.botDifficulty || kobposhChessBotProfile)
    kobposhChessBotPvCandidates = []

    if(kobposhChessBotProfile == "weak"){
        movetime = Number(options.movetime || 520)
        playerdepth = Number(options.playerdepth || 1)
        stockfish.postMessage("setoption name Skill Level value 2")
        stockfish.postMessage("setoption name UCI_LimitStrength value true")
        stockfish.postMessage("setoption name UCI_Elo value 900")
        stockfish.postMessage("setoption name MultiPV value 4")
    }else{
        movetime = Number(options.movetime || 2200)
        playerdepth = Number(options.playerdepth || 2)
...
function rememberKobposhChessPv(eventText){
    if(kobposhChessBotProfile != "weak") return
    var parts = String(eventText || "").trim().split(/\s+/)
    if(parts[0] != "info") return
    var multipvIndex = parts.indexOf("multipv")
    var pvIndex = parts.indexOf("pv")
    if(multipvIndex < 0 || pvIndex < 0 || pvIndex + 1 >= parts.length) return
    var rank = parseInt(parts[multipvIndex + 1])
    var move = String(parts[pvIndex + 1] || "").trim()
    if(!rank || move.length < 4 || move == "(none)") return
    kobposhChessBotPvCandidates[rank - 1] = move
}

function chooseKobposhChessBotMove(bestmove){
    if(kobposhChessBotProfile != "weak") return bestmove
    var fallback = String(bestmove || "").trim()
    if(fallback.length < 4 || fallback == "(none)") return fallback

    var candidates = []
    for(var i = 0; i < kobposhChessBotPvCandidates.length; i++){
        var move = String(kobposhChessBotPvCandidates[i] || "").trim()
        if(move.length >= 4 && candidates.indexOf(move) < 0){
    ...
    kobposhChessBotMoveCounter++
    var roll = Math.random()
    var chosen = fallback
    if(kobposhChessBotMoveCounter % 6 == 0){
        chosen = fallback
    }else if(candidates[2] && roll < 0.32){
        chosen = candidates[2]
    }else if(candidates[1] && roll < 0.82){
        chosen = candidates[1]
    }
    return chosen || fallback
}

window.configureKobposhChessBot = configureKobposhChessBot
        ...
        if(event.split(" ")[0] == "bestmove"){
            if(canMove){
                ...
                if(playercolor != turn && isai){
                    bestmove = chooseKobposhChessBotMove(event.split(" ")[1])
