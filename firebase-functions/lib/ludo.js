...
function isLudoBotPilotResult(result = {}) {
  const roomMode = String(result?.roomMode || result?.gameMode || result?.mode || "").trim().toLowerCase();
  if (roomMode === "ludo_friends" || roomMode.includes("friend")) return false;
  if (roomMode === "ludo_local_bot" || roomMode.includes("bot") || roomMode.includes("ai")) return true;
  return safeInt(result?.botCount) > 0 && safeInt(result?.humanCount) <= 1;
}
...
async function computeLudoBotPilotSnapshot(options = {}) {
  ...
  querySnap.forEach((docSnap) => {
    ...
    if (!isLudoBotPilotResult(data)) return;
